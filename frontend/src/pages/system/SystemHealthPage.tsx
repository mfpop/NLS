...
function statusBadge(status: HealthStatus): string {
  switch (status) {
    case "healthy": return "bg-success/15 text-success border-success/20";
    case "warning": return "bg-warning/15 text-warning border-warning/20";
    case "critical": return "bg-danger/15 text-danger border-danger/20";
...
function Row({ label, value, status }: { label: string; value: string; status?: HealthStatus }) {
  return (
    <div className="h-8 px-3 border-b border-border/50 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <span className="text-xs text-muted-foreground font-medium truncate max-w-[180px]">{value}</span>
      </div>
    </div>
  );
}
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="h-8 px-3 border-b border-border/50 flex items-center justify-between">
      <span className="min-w-0 truncate text-xs text-muted-foreground">{label}</span>
      <span className="text-xs text-muted-foreground font-mono truncate max-w-[180px]">{value}</span>
    </div>
  );
}
...
function PanelHeader({ icon: Icon, title }: { icon: React.ComponentType<{ className?: string }>; title: string }) {
  return (
    <div className="h-8 border-b border-border bg-muted px-3 flex items-center gap-2 shrink-0">
...
function StatusStrip({ health }: { health: SystemHealthData["systemHealth"] }) {
  ...
  return (
    <div className="grid grid-cols-6 divide-x divide-border border-b border-border h-16 shrink-0">
      {items.map((item) => (
        <div key={item.label} className="flex flex-col items-center justify-center gap-1 px-2">
          ...
          {item.status ? (
            ...
            <span className="text-xs text-muted-foreground font-medium truncate max-w-[120px]">{item.value}</span>
          )}
        </div>
      ))}
    </div>
...
function HealthSummaryView({ health }: { health: SystemHealthData["systemHealth"] }) {
  return (
    <div className="flex flex-col min-h-0">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-2 grid-rows-2 divide-x divide-y divide-border" style={{ gridTemplateRows: "1fr 1fr" }}>
        {/* Application & API */}
        ...
        <div className="flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
            {health.services.length === 0 ? (
              <div className="flex items-center justify-center h-16 text-xs text-muted-foreground/60">No services reported.</div>
            ) : (
        ...
        <div className="flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
            {health.checks.length === 0 ? (
              <div className="flex items-center justify-center h-16 text-xs text-muted-foreground/60">No checks available.</div>
            ) : (
...
function ServicesView({ health }: { health: SystemHealthData["systemHealth"] }) {
  ...
  return (
    <div className="flex flex-col min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {health.services.length === 0 ? (
          <div className="flex items-center justify-center h-24 text-xs text-muted-foreground/60">No services reported.</div>
        ) : (
          health.services.map((svc) => (
            <div key={svc.name} className="h-9 px-3 border-b border-border/50 flex items-center justify-between">
              <span className="min-w-0 truncate text-xs text-muted-foreground">{svc.name}</span>
              <div className="flex items-center gap-2 shrink-0">
                ...
                <span className="text-xs text-muted-foreground font-medium truncate max-w-[180px]">{svc.detail}</span>
              </div>
            </div>
          ))
        )}
...
function DeploymentView({ health }: { health: SystemHealthData["systemHealth"] }) {
  return (
    <div className="flex flex-col min-h-0">
      <div className="h-8 border-b border-border bg-muted px-3 flex items-center shrink-0">
        <span className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Deployment Info</span>
      </div>
      <div className="flex-1 min-h-0 overflow-y-auto">
...
function ErrorsView({ health }: { health: SystemHealthData["systemHealth"] }) {
  return (
    <div className="flex flex-col min-h-0">
      <div className="h-8 border-b border-border bg-muted px-3 flex items-center shrink-0">
        <span className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Recent Errors</span>
      </div>
      <div className="flex-1 min-h-0 overflow-y-auto">
        {health.recentErrors.length === 0 ? (
          <div className="flex items-center justify-center h-24 text-xs text-muted-foreground/60">No recent errors reported.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-border bg-muted text-left text-[10px] font-medium text-muted-foreground uppercase tracking-wide">
              ...
              <tbody>
                {health.recentErrors.map((err, idx) => (
                  <tr key={idx} className="border-b border-border/50 hover:bg-muted/50">
                    <td className="px-3 py-1.5 text-muted-foreground whitespace-nowrap">{formatTime(err.timestamp)}</td>
                    <td className="px-3 py-1.5">
                      <span className={`inline-flex items-center px-1.5 py-0.5 text-[10px] font-medium rounded-sm border whitespace-nowrap ${err.severity === "error" ? "bg-danger/15 text-danger border-danger/20" : err.severity === "warning" ? "bg-warning/15 text-warning border-warning/20" : "bg-muted text-muted-foreground border-border"}`}>
                        {err.severity}
                      </span>
                    </td>
                    <td className="px-3 py-1.5 text-muted-foreground truncate max-w-[120px]">{err.source}</td>
                    <td className="px-3 py-1.5 text-muted-foreground truncate max-w-[300px]">{err.message}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <TwoColumnPageTemplate
      ...
      leftPanelProps={{
        ...
        renderRecord: (s, _selected) => {
          ...
          return (
            <div className="flex items-center gap-2 py-0.5">
              ...
              <span className="flex-1 text-xs text-muted-foreground truncate">{s.label}</span>
            </div>
          );
        },
        ...
        selectedBgClass: "bg-success/10/40",
      }}
      footerLeft="System Health"
      footerCenter={
        health ? (
          <span className="flex items-center gap-4">
            <span>Overall: <span className={health.overallStatus === "healthy" ? "text-success" : health.overallStatus === "warning" ? "text-warning" : health.overallStatus === "critical" ? "text-danger" : "text-muted-foreground"}>{statusLabel(health.overallStatus)}</span></span>
            {health.deploymentInfo.serverTime && (
              <><span className="h-4 w-px bg-muted/80" /><span>Updated: {formatTime(health.deploymentInfo.serverTime)}</span></>
            )}
          </span>
        ) : "Checking health..."
      }
    >
      <div className="h-full bg-muted">
        {loading && !health ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-xs text-muted-foreground/60">
            <div className="h-8 w-8 rounded-full bg-muted flex items-center justify-center animate-pulse">
              <Activity className="h-4 w-4 text-muted-foreground/60" />
            </div>
            <span className="text-sm font-medium text-muted-foreground">Loading health data...</span>
          </div>
        ) : error && !health ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-xs text-muted-foreground/60">
            ...
            <span className="text-sm font-medium text-muted-foreground">Unable to load health data</span>
            <span className="text-muted-foreground/60 max-w-xs text-center">{error.message}</span>
            <button type="button" onClick={() => refetch()} className="mt-1 inline-flex items-center gap-1.5 rounded-sm bg-success/10 px-3 py-1.5 text-xs font-medium text-success hover:bg-success/15 transition-colors">
              <RefreshCw className="h-3 w-3" />
              Retry
            </button>
          </div>
        ) : !health ? (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-xs text-muted-foreground/60">
            <span>Unable to load health data.</span>
            <button type="button" onClick={() => refetch()} className="inline-flex items-center gap-1.5 rounded-sm bg-muted px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted/80 transition-colors">
              <RefreshCw className="h-3 w-3" />
              Retry
            </button>
          </div>
