...
class ImportSourceConfigService:
    """Configure ERP/Excel import file locations only — no import execution."""
    ...
    def _find_active_duplicate(*, name: str | None = None, domain: str | None = None, source_type: str | None = None,
                               path: str | None = None, file_pattern: str | None = None, exclude_id: int | None = None):
    ...
    @staticmethod
    @transaction.atomic
    def create_config(data: dict[str, Any]) -> ImportSourceConfig:
        ImportSourceConfigService._validate_required_fields(data, partial=False)
        ...
        if is_active:
            dup = ImportSourceConfigService._find_active_duplicate(name=name, domain=domain, source_type=source_type, path=path, file_pattern=file_pattern)
            if dup:
    ...
    @staticmethod
    @transaction.atomic
    def update_config(config_id: int, data: dict[str, Any]) -> ImportSourceConfig:
        ...
        # Build prospective values using existing config and incoming partial data
        ImportSourceConfigService._validate_required_fields(data, partial=True)
        ...
        # If resulting config will be active, ensure no duplicate active exists
        if new_is_active:
            dup = ImportSourceConfigService._find_active_duplicate(name=new_name, domain=new_domain, source_type=new_source_type, path=new_path, file_pattern=new_file_pattern, exclude_id=config.id)
            if dup:
                raise ImportSourceConfigError("name", "Duplicate active import source exists", code="DUPLICATE")
    ...
    @staticmethod
    @transaction.atomic
    def archive_config(config_id: int) -> ImportSourceConfig:
        config = ImportSourceConfig.objects.select_for_update().get(pk=config_id)
    ...
    @staticmethod
    def sanitize_for_export(config: ImportSourceConfig) -> dict[str, Any]:
        """Diagnostics-safe snapshot — paths only, no secrets."""
        return {
            "id": str(config.id),
            "name": config.name,
