...
export function MaterialOverview(props: OverviewProps) {
  ...
  const kpiCards: KpiCard[] = useMemo(() => [
    { label: "Material Audits Open", count: kpis.openAudits.length, color: "text-primary dark:text-blue-400", sub: `${kpis.openAudits.length} pending review`, onClick: () => kpiClick({ tab: "audits", status: "OPEN" }) },
    { label: "Open Problems", count: kpis.openIssues.length, color: "text-warning dark:text-amber-400", sub: "Requires action", trend: kpis.openIssues.length > 3 ? "up" : kpis.openIssues.length === 0 ? "down" : "neutral", trendVal: `${kpis.openIssues.length > 0 ? "+" : ""}${kpis.openIssues.length}`, onClick: () => kpiClick({ tab: "issues", status: "OPEN" }) },
      ...
      <div className="bg-background/60 dark:bg-slate-900/60 backdrop-blur-md border border-white/30 dark:border-slate-700/30 p-4">
        ...
        <div className="flex items-center justify-around gap-4">
          ...
          <div className="flex flex-col gap-1.5 text-xs">
            ...
            <div className="flex items-center gap-2">
              <span className="h-2 w-2 rounded-full bg-danger/100" />
          ...
          <div className="bg-background/60 dark:bg-slate-900/60 backdrop-blur-md border border-white/30 dark:border-slate-700/30 p-3">
            ...
            {riskItems.length === 0 ? (
              <div className="flex items-center justify-center h-20 text-xs text-muted-foreground italic">
                <span className="text-success mr-1.5">✓</span> No material risks need attention
              </div>
            ) : (
            ...
            {dueThisWeek.length === 0 ? (
              ...
              <div className="space-y-0.5">
                {dueThisWeek.map((item) => {
                  ...
                  return (
                    <button key={item.id} onClick={item.onClick}
                      className={`w-full text-left flex items-center gap-2 text-xs py-1.5 px-1 border-b border-white/10 dark:border-slate-700/10 last:border-b-0 hover:bg-background/40 dark:hover:bg-slate-800/40 transition-colors cursor-pointer ${isToday ? "bg-warning/10/40 dark:bg-amber-950/20" : ""}`}
                    >
          ...
          <div className="bg-background/60 dark:bg-slate-900/60 backdrop-blur-md border border-white/30 dark:border-slate-700/30 p-3">
            ...
            {kpis.openAudits.length === 0 ? (
              <div className="flex items-center justify-center h-16 text-xs text-muted-foreground italic">
                <span className="text-success mr-1.5">✓</span> All audits completed
              </div>
            ) : (
              <div className="space-y-0.5">
                  ...
                  return (
                    ...
                    >
                      <span className={`w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-bold ${
                        ...
                        : item.type === "Problem" ? "bg-warning/15 text-warning dark:bg-amber-900/30 dark:text-amber-300"
      ...
      {auditTemplates && auditTemplates.length === 0 && onInstallTemplates && (
        <div className="bg-warning/10/80 dark:bg-amber-950/80 backdrop-blur-sm border border-warning/20/50 dark:border-amber-800/50 p-4 text-center rounded-lg">
          <div className="flex items-center justify-center gap-1.5 mb-1">
            ...
            <p className="text-xs font-semibold text-warning dark:text-amber-400">No material audit templates installed</p>
          </div>
          <p className="text-[10px] text-warning/70 dark:text-amber-400/70 mb-2">Install the default template pack to get started with material control audits.</p>
