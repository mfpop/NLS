...
import { RefreshCw, Clock, AlertCircle, ListChecks, Activity, Workflow, Footprints, Download } from "lucide-react";
...
export function LiveShopfloorToolbar({
  onRefresh,
  onLogDowntime,
  onNewIssue,
  onNewAction,
  ...
  return (
    <Toolbar
      ...
      right={
        <>
          <ToolbarButton icon={RefreshCw} label={refreshing ? "Refreshing..." : "Refresh"} onClick={onRefresh} disabled={!allowRefresh || refreshing} title="Refresh live shopfloor data" />
          ...
          <span className="mx-0.5 h-5 w-px shrink-0 bg-border/30" />
          <ToolbarButton icon={Activity} label="Line Perf." onClick={() => navigate("/execution/line-performance")} title="Go to Line Performance" />
