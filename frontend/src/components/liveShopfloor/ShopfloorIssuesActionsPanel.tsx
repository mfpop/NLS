...
export function ShopfloorIssuesActionsPanel({ issues, actions, onNewIssue, onNewAction }: Props) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden border-b border-border">
      {/* Header */}
      <div className="h-8 shrink-0 border-b border-border px-3 flex items-center justify-between bg-muted">
        <div className="flex items-center gap-1.5 min-w-0">
          <ListChecks className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
          <h3 className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide truncate">Issues & Actions</h3>
          {hasOverdue && (
            <span className="shrink-0 inline-flex items-center gap-1 rounded-full bg-danger/10 px-1.5 py-0.5 text-[9px] font-medium text-danger border border-danger/20">
              <span className="h-1.5 w-1.5 rounded-full bg-danger/100" />
              Overdue
            </span>
          )}
          <span className="text-[10px] text-muted-foreground/60 tabular-nums shrink-0">({allItems.length})</span>
        </div>
                  ...
                  <div key={issue.id} className="group flex items-start gap-2 px-3 py-1.5 border-b border-border/50 last:border-b-0 hover:bg-background transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5 text-[10px] text-muted-foreground flex-wrap">
                        ...
                        {issue.owner && <><span className="text-muted-foreground/30">·</span><User className="h-2.5 w-2.5 shrink-0" /><span className="truncate max-w-[90px]">{issue.owner}</span></>}
                        {issue.dueDate && <><span className="text-muted-foreground/30">·</span><Calendar className="h-2.5 w-2.5 shrink-0" /><span className={isOverdue(issue.dueDate) ? "text-danger font-medium" : ""}>{issue.dueDate}</span></>}
                        {issue.linkedResourceName && <><span className="text-muted-foreground/30">·</span><span className="truncate max-w-[70px]">{issue.linkedResourceName}</span></>}
                      </div>
                      <div className="hidden group-hover:flex items-center gap-1 mt-1">
                        ...
                        <button type="button" className="text-[9px] text-muted-foreground hover:bg-muted rounded px-1.5 py-0.5 font-medium transition-colors">Close</button>
                      </div>
                    </div>
                  </div>
                );
              ...
              return (
                <div key={action.id} className="group flex items-start gap-2 px-3 py-1.5 border-b border-border/50 last:border-b-0 hover:bg-background transition-colors">
                  <ArrowRight className={`h-3.5 w-3.5 shrink-0 mt-0.5 ${
                    overdue ? "text-danger" : action.priority === "urgent" ? "text-danger"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5 text-[10px] text-muted-foreground flex-wrap">
                      ...
                      {action.assignedTo && <><span className="text-muted-foreground/30">·</span><User className="h-2.5 w-2.5 shrink-0" /><span className="truncate max-w-[90px]">{action.assignedTo}</span></>}
                      {action.dueDate && <><span className="text-muted-foreground/30">·</span><Calendar className="h-2.5 w-2.5 shrink-0" /><span className={overdue ? "text-danger font-medium" : ""}>{action.dueDate}</span></>}
                      {action.linkedResourceName && <><span className="text-muted-foreground/30">·</span><span className="truncate max-w-[70px]">{action.linkedResourceName}</span></>}
                    </div>
                    <div className="hidden group-hover:flex items-center gap-1 mt-1">
                      <button type="button" className="text-[9px] text-accent-foreground hover:bg-accent/10 rounded px-1.5 py-0.5 font-medium transition-colors">Assign</button>
                      <button type="button" className="text-[9px] text-warning hover:bg-warning/10 rounded px-1.5 py-0.5 font-medium transition-colors">Escalate</button>
