...
export interface NodeDetailPanelProps {
  ...
  onAddChild?: () => void;
  onSave?: (data: { name: string; code: string; status?: string }) => void;
}
...
export function NodeDetailPanel({
  ...
  if (workspaceMode === "create") {
    return <CreateContent node={selectedNode} onSave={onSave} />;
  }
...
function ViewContent({
  ...
  return (
    <div>
      ...
      <DetailSection title="Related Configuration" bodyClass="px-0 py-0">
        <div className="flex items-center gap-2 h-9 text-[13px] text-slate-400 dark:text-slate-500">
          <Database className="h-3.5 w-3.5 stroke-current shrink-0" />
          <span>No configuration links for this entity yet.</span>
          <button type="button" onClick={() => navigate("/system/production-structure/references")} className="ml-auto text-[13px] font-medium text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 transition-colors">
            Configure
          </button>
        </div>
      </DetailSection>
      ...
      {(() => {
        ...
        return (
          <DetailSection title="Configuration Shortcuts" bodyClass="px-0 py-0">
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {actions.map((a) => (
                <div key={a.label} className="flex items-center justify-between min-h-10 text-[14px] hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <button type="button" onClick={() => { if (a.isAdd && onAddChild) onAddChild(); else if (a.route) navigate(a.route); }} className="shrink-0 h-6 px-2 rounded text-[11px] font-medium text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 transition-colors">
                    {a.isAdd ? "Create" : "Open"}
                  </button>
                </div>
              ))}
