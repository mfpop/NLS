...
function areaLabel(ca: string): string {
  const m: Record<string, string> = { "QUALITY": "Quality", "SAFETY": "Safety", "MATERIAL": "Material Control", "PRODUCTION_CONTROL": "Production Control" };
  return m[ca] || "Quality";
}
function accentCls(ca: string): string {
  const m: Record<string, string> = { "QUALITY": "border-cyan-500", "SAFETY": "border-orange-500", "MATERIAL": "border-teal-500", "PRODUCTION_CONTROL": "border-blue-500" };
  return m[ca] || "border-cyan-500";
}
...
export function useAuditSection(_search: string, filterStatus: string, activePlantId: string | null, productionLineId: string | null, onMessage: (msg: string, tone?: "success" | "error") => void, controlArea: string = "QUALITY", moduleScope: string = "QUALITY_CONTROL", installMutation?: DocumentNode) {
  ...
  const execQ = useQuery<any>(AUDIT_EXECUTION_FORM_QUERY, { variables: { auditId: String(execId ?? "") }, skip: !execId || creating, fetchPolicy: "cache-and-network" });
  ...
  // Auto-open finding dialog when FAIL/NO selected
  ...
  // ── Hooks ──
  // Sync state when an existing audit is selected — ensures readonly view, never stale "New Audit" heading
  useEffect(() => {
    if (execId != null) {
      setCreating(false);
      setCreated(true);
  ...
  const fmtErr = (msg: string | undefined | null): string => {
    ...
    if (msg.includes("created_at")) return "Audit could not be created — try again";
    if (msg.includes("duplicate") || msg.includes("already exists")) return "An audit with this configuration already exists";
    if (msg.includes("not found")) return "The record was not found — it may have been deleted";
    if (msg.includes("permission") || msg.includes("denied")) return "You don't have permission to perform this action";
    if (msg.includes("required")) return msg.charAt(0).toUpperCase() + msg.slice(1);
    return msg;
  };
  ...
  const hSaveDraft = useCallback(async () => {
    ...
    try {
      ...
      const items = Object.entries(ans).filter(([, a]) => {
        ...
        return v !== "" || c !== "";
      }).map(([qId, a]) => ({ questionId: Number(qId), answerValue: created ? (a as any).value : (a as any).v, comment: created ? (a as any).comment : (a as any).c })).filter((i) => Number.isFinite(i.questionId));
  ...
  const hComplete = useCallback(async () => {
    ...
    if (r.data?.completeAudit?.ok || r.data?.completeAudit?.audit?.id) { onMessage("Audit completed"); setCreating(false); setDraftAns({}); setLocalAns({}); auditsQ.refetch(); execQ.refetch(); }
  ...
  const renderList = (selId: number | null, onSelect: (id: number | null) => void) => (
    <div className="flex flex-col min-h-0 h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        : <div>{audits.map((a: any) => <div key={a.id} onClick={() => { onSelect(Number(a.id)); setCreating(false); setCreated(true); setExecId(Number(a.id)); setTplId(null); setErrors([]); }} className={`group mx-1 my-0.5 flex min-h-[52px] cursor-pointer items-center gap-2.5 px-3 transition-all duration-150 ${selId === Number(a.id) ? "bg-table-selected border-l-2 border-l-cyan-500" : "border-l-2 border-l-transparent hover:bg-table-row-hover"}`}>
          <div className="min-w-0 flex-1"><div className="grid min-w-0 items-center gap-2" style={{ gridTemplateColumns: "minmax(0,1fr) auto" }}><span className="min-w-0 truncate text-sm font-semibold text-foreground">{a.title || `Audit #${a.id}`}</span><span className={`inline-flex items-center px-1.5 py-0.5 text-[10px] font-semibold border ${STATUS_STYLES[a.status] || ""}`}>{statusLabel(a.status || "DRAFT")}</span></div><div className="flex items-center gap-1.5 text-xs text-muted-foreground flex-wrap"><span className="text-muted-foreground">{a.auditType || "-"}</span><span>·</span><span className="text-muted-foreground">{a.targetType || "-"}</span>{a.auditor && <><span>·</span><span className="text-muted-foreground">{a.auditor}</span></>}{a.auditDate && <><span>·</span><span className="text-muted-foreground">{a.auditDate}</span></>}{a.score !== null && a.score !== undefined && <><span>·</span><span className="font-mono text-muted-foreground">{a.score}%</span></>}</div></div>
        </div>)}</div>}
      </div>
    </div>
  );
