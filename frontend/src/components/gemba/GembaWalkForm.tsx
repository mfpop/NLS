...
export function GembaWalkForm({ sessionId, onSave, saving, readOnly, structureError, onRetryStructure, plantId, plantName, productionLineId, productionLineName }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="h-9 shrink-0 flex items-center gap-1.5 px-3 border-b border-border bg-muted">
        <Footprints className="h-4 w-4 text-muted-foreground" />
        <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">New Observation</h3>
      </div>

      {/* Form body */}
      <div className="flex-1 min-h-0 overflow-y-auto px-3 py-2.5 space-y-2.5">
        {readOnly && (
          <div className="rounded bg-warning/10 border border-warning/20 px-2 py-1.5 text-xs text-warning">
        ...
        {/* Focus */}
        <div>
          <label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground block mb-1">Focus</label>
          <input
            type="text"
            value={focus}
        ...
        <div className="grid grid-cols-3 gap-2">
          <div>
            <label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground block mb-1">Category *</label>
            <select
              value={category}
        ...
        <div>
          <label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground block mb-1">Linked Resource</label>
          <input
            type="text"
        ...
        {/* Error */}
        {error && (
          <div className="flex items-center gap-1.5 rounded bg-danger/10 border border-danger/20 px-2 py-1.5">
