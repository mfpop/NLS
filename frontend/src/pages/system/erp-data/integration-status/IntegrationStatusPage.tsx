...
const buttonClass = `inline-flex items-center gap-1.5 h-8 px-2 rounded text-[11px] font-medium text-emerald-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors`;

export function IntegrationStatusPage() {
  return (
    <AppPageLayout
      title="Integration Status"
      subtitle="Monitor integration health, connectivity status, and real-time data flow between ERP systems and the platform."
      icon={<Activity />}
      ...
      <div className="flex flex-col h-full overflow-hidden">
        <div className="shrink-0 flex h-9 items-center gap-2 border-b border-border/35 bg-muted px-3 select-none">
          <div className="relative w-72">
            ...
            <input type="text" placeholder="Search integration status..." className={inputClass} />
          </div>
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <Activity className="h-12 w-12 mx-auto text-emerald-600/30 stroke-current" />
            <p className="mt-3 text-sm text-muted-foreground">No integration sources configured.</p>
          </div>
        </div>
      </div>
    </AppPageLayout>
