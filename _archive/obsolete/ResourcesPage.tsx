...
import {
  ...
  PrimaryAction, ActionsDropdown, AlertBanner, SmartControlTowerLink
} from ""./shared"";
    ...
    <div className=""flex h-full flex-col overflow-hidden"" style={{ minHeight: 0 }}>
      <header className=""flex shrink-0 items-center gap-4 border-b border-[var(--border-soft)] bg-[var(--surface-1)] px-5 py-3"">
        ...
        <div className=""min-w-0 flex-1"">
          <h1 className=""text-base font-semibold tracking-tight text-[var(--text-primary)]"">Resources</h1>
          <p className=""text-xs text-[var(--text-secondary)]"">Manage machines, workstations, cells, tools, and production resources with live operational status.</p>
        </div>
      ...
      <div className=""flex-1 overflow-y-auto bg-[var(--page-bg)] p-4"">
        ...
        {downResources.length > 0 && (
          <AlertBanner
            message={downResources.length === 1 ? 1 resource down — affecting flow : ${downResources.length} resources down — affecting flow}
            ...
            {filtered.map((res) => {
              ...
              return (
                <div
                  ...
                  onClick={() => navigate(/system/data-management/resources/)}
                  role=""button"" tabIndex={0}
                  onKeyDown={(e) => { if (e.key === ""Enter"") navigate(/system/data-management/resources/); }}
                >
                  <div className=""flex items-center justify-between"">
                    <div className=""flex items-center gap-3 min-w-0"">
                      ...
                      <div className=""min-w-0"">
                        <div className=""flex items-center gap-2"">
                          ...
                          {res.utilization > 100 && <span className=""text-[10px] font-bold text-red-600"">OVERLOAD</span>}
                        </div>
                    ...
                    <div className=""hidden items-center gap-2 sm:flex"" onClick={(e) => e.stopPropagation()}>
                      ...
                      <PrimaryAction onClick={() => navigate(/system/data-management/resources/)} />
                      <ActionsDropdown actions={[
                        { label: ""Edit"", icon: <Pencil className=""h-3 w-3"" />, onClick: () => {} },
                        { label: ""Assign to group"", icon: <ExternalLink className=""h-3 w-3"" />, onClick: () => {} },
                        { label: ""View in VSM"", icon: <GitBranch className=""h-3 w-3"" />, onClick: () => navigate(/execution/vsm?resource=) },
                        { label: ""View in Control Tower"", icon: <ExternalLink className=""h-3 w-3"" />, onClick: () => navigate(/control-tower?resource=&plant=&line=) },
                      ]} />
                    </div>
                  </div>
                </div>
