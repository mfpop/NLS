    ...
    <g onClick={() => onSelect(node.id)} className="cursor-pointer" role="button" tabIndex={0}>
      ...
      {isBn && (
        <rect x={x - 4} y={y - 4} width={width + 8} height={height + 8} rx={5}
          fill="none" stroke="#f59e0b" strokeWidth={1.5} strokeOpacity={0.3} strokeDasharray="4,3" />
      )}
      ...
      <text x={x + width / 2} y={y + 20} textAnchor="middle" className="text-sm font-semibold" fill="#1e293b">
        {node.label}
      </text>

      <text x={x + width / 2} y={y + 35} textAnchor="middle" className="text-[11px]" fill="#64748b">
        {node.resourceGroupName}
      </text>

      {isBn && (
        <g>
          <rect x={x + 6} y={y + height - 19} width={width - 12} height={15} rx={2} fill="#f59e0b" opacity={0.92} />
          <text x={x + width / 2} y={y + height - 9} textAnchor="middle" className="text-[8px] font-bold" fill="#ffffff">
            ⚠ BOTTLENECK
          </text>
        </g>
      )}
      ...
      {isStarved && (
        <text x={x + width - 4} y={y + height - 2} textAnchor="end" className="text-[7px] italic" fill="#f97316">starved</text>
      )}
      {isOverproduced && (
        <text x={x + width - 4} y={y + height - 2} textAnchor="end" className="text-[7px] italic" fill="#ef4444">overprod</text>
      )}
      ...
      {dataBoxHeight > 16 && (
        <>
          ...
          <text x={x + 6} y={dbY + 14} className="text-[11px]" fill="#475569">CT={node.cycleTimeSeconds}s</text>
          <text x={x + width - 6} y={dbY + 14} textAnchor="end" className="text-[11px]" fill="#475569">C/O={fmtCO(node.changeoverSeconds)}</text>

          <text x={x + 6} y={dbY + 27} className="text-[11px]" fill="#475569">Up={node.uptimePercent}%</text>
          ...
          {dataBoxHeight > 32 && (
            <svg x={x + 6} y={dbY + 30} width={width - 12} height={4} overflow="visible">
              <rect x={0} y={0} width={width - 12} height={4} rx={2} fill="#e2e8f0" />
              <rect x={0} y={0}
