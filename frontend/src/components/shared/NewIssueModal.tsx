...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: NewIssueFormData) => void;
  saving: boolean;
  lineName: string;
  shiftName: string;
  sourceType: string;
...
export function NewIssueModal({ isOpen, onClose, onSave, saving, lineName, shiftName, sourceType, initialTitle, initialDescription }: Props) {
  ...
  return (
    <div ref={overlayRef} onClick={handleBackdropClick} className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/45" role="dialog" aria-modal="true" aria-label="New issue">
      <div className="w-full max-w-lg rounded-lg border border-border bg-card shadow-xl">
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Describe the abnormal condition or problem..."
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide block mb-1">Severity</label>
          ...
          <div>
            <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide block mb-1">Owner</label>
            <input
              type="text"
              value={owner}
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-border/20">
          ...
          <button
            type="button"
            onClick={() => onSave({ title: title.trim(), description, severity, owner, dueDate })}
