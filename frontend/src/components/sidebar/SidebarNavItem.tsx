...
export function SidebarNavItem({ to, icon: Icon, label, depth = 0, onNavigate }: {
  ...
  return (
    <NavLink to={to} end={to === "/"} onClick={onNavigate}
      className={({ isActive }) =>
        `flex items-center gap-2.5 h-8 text-xs font-medium transition-colors pr-3 ${
          isActive
            ? "bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100 border-l-[3px] border-emerald-500 dark:border-emerald-400"
