  ...
  return (
    <div className={`flex h-full flex-col overflow-hidden ${theme.page}`} style={{ minHeight: 0 }}>
              ...
              return (
                <div key={res.id} ref={idx === 0 ? cardRef : undefined}>
                  <DataCard
                    ...
                    onEdit={() => openEdit(res)}
                    onOpen={() => navigate(`/system/data-management/resources/${res.id}`)}
                    isLowestLevel
                  />
                </div>
              );
      ...
      <UnifiedModal
        ...
        onConfigureStructure={editingResource ? () => navigate(`/system/data-management/structure?resource=${encodeURIComponent(editingResource.name)}`) : undefined}
      />
