  ...
  return (
    <div ref={ref} className={`relative shrink-0 border-t ${layoutBg.horizontalSeparator} ${layoutBg.sidebar} text-sidebar-foreground`}>
      ...
      <button type="button" onClick={() => setIsOpen(!isOpen)}
        className="flex items-center gap-2.5 w-full h-10 px-3 text-[13px] font-medium text-sidebar-foreground hover:bg-sidebar-hover transition-colors outline-none focus-visible:ring-1 focus-visible:ring-sidebar-control"
      >
        ...
        <span className="flex flex-col flex-1 min-w-0 text-left leading-snug">
          ...
          <span className="truncate text-[10px] font-medium text-sidebar-muted">{user?.role?.replace("_", " ") ?? ""}</span>
        </span>
        <MoreVertical className="h-3.5 w-3.5 stroke-current shrink-0 text-sidebar-muted" />
      </button>
    </div>
  );
}
