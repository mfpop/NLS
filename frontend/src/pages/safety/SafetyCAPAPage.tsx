...
export function SafetyCAPAPage() {
  ...
  const toolbarContent = (
    <ExplorerToolbar
      ...
      actions={
        ...
        !selItem ? (<><ExplorerToolbarButton icon={Plus} label="New CAPA" onClick={hNew} variant="success" /><ExplorerToolbarButton icon={RefreshCw} label="Refresh" onClick={() => refetch()} /></>) :
        (<><ExplorerToolbarButton icon={Pencil} label="Edit" onClick={() => hEdit(selItem)} disabled={!isEditable} /><ExplorerToolbarButton icon={Play} label="Open" onClick={() => hTransition(openMut, selItem.id, "CAPA opened")} disabled={!canOpen} /><ExplorerToolbarButton icon={Play} label="Start" onClick={() => hTransition(startMut, selItem.id, "CAPA started")} disabled={!canStart} /><ExplorerToolbarButton icon={Target} label="Pending Eff." onClick={() => hTransition(pendingEffMut, selItem.id, "Pending effectiveness")} disabled={!canPendingEff} /><ExplorerToolbarButton icon={RotateCcw} label="Complete Eff." onClick={() => setEffectivenessPopup({ id: selItem.id })} disabled={!canCompEff} /><ExplorerToolbarButton icon={CheckCircle} label="Close" onClick={() => hTransition(closeMut, selItem.id, "CAPA closed")} disabled={!canClose} variant={canClose ? "success" : "default"} /><ExplorerToolbarButton icon={XCircle} label="Cancel" onClick={() => setCancelId(selItem.id)} disabled={!canCancel} variant="destructive" /><ExplorerToolbarSeparator /><ExplorerToolbarButton icon={ArrowLeft} label="Back" onClick={() => { setSelectedId(null); setCreating(false); setEditing(false); }} /><ExplorerToolbarButton icon={RefreshCw} label="Refresh" onClick={() => refetch()} /></>)
      }
    />
  );
  ...
  const leftColumnContent = (
    <><div className="shrink-0 h-8 border-b border-slate-200 flex items-center bg-slate-50 px-4"><span className="text-sm font-medium text-slate-700">CAPAs</span><span className="ml-auto text-[10px] text-slate-500 font-mono">{items.length}</span></div>      {searchQuery && filteredItems.length === 0 && items.length > 0 && <div className="px-4 py-2 text-[10px] text-slate-400 italic">No CAPAs match &quot;{searchQuery}&quot;</div>}
      <div className="flex-1 overflow-y-auto divide-y divide-slate-100">{filteredItems.length === 0 ? (<div className="flex flex-col items-center justify-center px-4 py-8 text-center"><Search className="h-5 w-5 text-slate-300 mb-2" /><p className="text-xs text-slate-500 font-medium">No CAPA records recorded.</p><button onClick={hNew} className="mt-3 h-7 px-3 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700">New CAPA</button></div>) : filteredItems.map((e: any) => (<div key={e.id} onClick={() => { setCreating(false); setEditing(false); setSelectedId(e.id); }} className={`group flex items-start gap-2 w-full rounded-md px-3 py-2.5 cursor-pointer text-sm transition-all border-l-2 ${selectedId === e.id ? "bg-accent/15 border-accent" : "border-l-transparent hover:bg-muted"}`}><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="min-w-0 flex-1 truncate font-semibold text-slate-900">{e.title}</span></div><div className="flex items-center gap-2 mt-0.5 text-[11px] text-slate-500"><span className={`inline-flex items-center px-1 py-0.5 text-[9px] font-medium border ${STATUS_STYLES[e.status] || STATUS_STYLES.DRAFT}`}>{statusLabel(e.status)}</span>{e.owner && <span>· {e.owner}</span>}</div></div></div>))}</div></>
  );
