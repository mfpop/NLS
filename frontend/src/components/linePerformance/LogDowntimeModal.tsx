...
import type { LinePerformanceFilters } from "@/types/linePerformance";

interface Props {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: LogDowntimeFormData) => void;
  saving: boolean;
  filters: LinePerformanceFilters | null;
  lineName: string;
  shiftName: string;
}

export interface LogDowntimeFormData {
  reasonId: string;
  startTime: string;
  endTime: string;
  description: string;
...
export function LogDowntimeModal({ isOpen, onClose, onSave, saving, filters, lineName, shiftName }: Props) {
    ...
    <div ref={overlayRef} onClick={handleBackdropClick} className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/45" role="dialog" aria-modal="true" aria-label="Log downtime">
      <div className="w-full max-w-md rounded-lg border border-border bg-card shadow-xl">
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-border/20">
          ...
          <button
            type="button"
            onClick={() => onSave({ reasonId, startTime, endTime, description })}
