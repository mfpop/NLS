...
export function SafetyOverview(props: OverviewProps) {
  ...
  const kpis = useMemo(() => {
    ...
    // Since this is Safety, count unresolved safety issues specifically
    const safetyIssues = problems.filter((p) => p.problemType === "SAFETY" || p.severity === "CRITICAL" || p.severity === "HIGH");
    return { openIssues, openActions, openAudits, overdueActions, criticalHigh, completedAudits, completionRate, avgScore, recentCompleted, safetyIssues };
  ...
  const scores = useMemo(() => {
    const general = audits.filter((a) => a.auditType === "SF_GENERAL_SAFETY" || a.auditType === "SAFETY" || a.auditType === "SF_OBSERVATION");
    const ppe = audits.filter((a) => a.auditType === "SF_PPE_CHECK");
    const hazards = audits.filter((a) => a.auditType === "SF_UNSAFE_CONDITION" || a.auditType === "SF_HAZARD_IDENTIFICATION");
    const nearMiss = audits.filter((a) => a.auditType === "SF_NEAR_MISS");
    const machine = audits.filter((a) => a.auditType === "SF_MACHINE_GUARDING");
    const loto = audits.filter((a) => a.auditType === "SF_LOCKOUT_TAGOUT" || a.auditType === "LOTO");
    const emerg = audits.filter((a) => a.auditType === "SF_EMERGENCY_READINESS" || a.auditType === "SF_EMERGENCY_PREP");
    const ergo = audits.filter((a) => a.auditType === "SF_ERGONOMICS" || a.auditType === "SF_ERGONOMICS_CHECK");
    ...
    return {
      general: calcAvg(general),
      ppe: calcAvg(ppe),
      hazards: calcAvg(hazards),
      nearMiss: calcAvg(nearMiss),
      machine: calcAvg(machine),
      loto: calcAvg(loto),
      emergency: calcAvg(emerg),
      ergonomics: calcAvg(ergo),
  ...
  const alertBanner = useMemo(() => {
    if (kpis.criticalHigh.length > 2) return { msg: `${kpis.criticalHigh.length} critical safety issue${kpis.criticalHigh.length > 1 ? "s" : ""} require immediate attention!`, severity: "critical" as const };
    if (kpis.overdueActions.length > 3) return { msg: `${kpis.overdueActions.length} safety action${kpis.overdueActions.length > 1 ? "s" : ""} past due`, severity: "warning" as const };
    if (kpis.overdueActions.length > 0) return { msg: `${kpis.overdueActions.length} overdue safety action${kpis.overdueActions.length > 1 ? "s" : ""}`, severity: "info" as const };
    return null;
  }, [kpis]);
  ...
  // Human-readable audit type names
  const auditTypeName = (t: string) => {
    const names: Record<string, string> = {
      SF_GENERAL_SAFETY: "Safety Audit", SF_PPE_CHECK: "PPE Check", SF_UNSAFE_CONDITION: "Unsafe Condition",
      SF_NEAR_MISS: "Near Miss", SF_MACHINE_GUARDING: "Machine Guarding", SF_LOCKOUT_TAGOUT: "LOTO",
      SF_EMERGENCY_READINESS: "Emergency Readiness", SF_ERGONOMICS: "Ergonomics",
      SF_OBSERVATION: "Safety Observation", SF_PPE_COMPLIANCE: "PPE Compliance",
      SF_HAZARD_IDENTIFICATION: "Hazard ID", SF_EMERGENCY_PREP: "Emergency Prep", SF_ERGONOMICS_CHECK: "Ergonomics",
    };
    return names[t] || t.replace(/_/g, " ");
  };
    ...
    <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-4">
      ...
      {alertBanner && (
        ...
        >
          ...
          <button onClick={() => kpiClick({ tab: "issues" })} className="underline font-semibold hover:no-underline shrink-0">View</button>
        </div>
      )}
      ...
      <div className="bg-background/60 dark:bg-slate-900/60 backdrop-blur-md border border-white/30 dark:border-slate-700/30 p-4">
        ...
        <div className="flex items-center justify-around gap-4">
          ...
          <div className="relative flex flex-col items-center">
            <Gauge value={scores.general ?? 0} label="Safety Audit" color={gaugeColor(scores.general)} size="sm" />
          </div>
          <div className="relative flex flex-col items-center">
            <Gauge value={scores.ppe ?? 0} label="PPE Compliance" color={gaugeColor(scores.ppe)} size="sm" />
          </div>
          <div className="relative flex flex-col items-center">
            <Gauge value={scores.hazards ?? 0} label="Hazard Control" color={gaugeColor(scores.hazards)} size="sm" />
          </div>
          <div className="relative flex flex-col items-center">
            <Gauge value={scores.nearMiss ?? 0} label="Near Miss" color={gaugeColor(scores.nearMiss)} size="sm" />
          </div>
        ...
        {/* All audit types mini-scores */}
        {auditTypeBreakdown.length > 0 && (
          <div className="mt-4 pt-3 border-t border-white/10 dark:border-slate-700/10">
            <p className="text-[10px] font-medium text-muted-foreground mb-2">Safety Audit Type Scores</p>
        ...
        {/* Below gauges: Machine Guarding, LOTO, Emergency, Ergonomics */}
        <div className="mt-4 pt-3 border-t border-white/10 dark:border-slate-700/10">
          <p className="text-[10px] font-medium text-muted-foreground mb-2">Specialty Audits</p>
          <div className="flex items-center justify-around">
            {[
              { code: "machine", label: "Machine Guarding", value: scores.machine },
              { code: "loto", label: "Lockout/Tagout", value: scores.loto },
              { code: "emergency", label: "Emergency Readiness", value: scores.emergency },
              { code: "ergonomics", label: "Ergonomics", value: scores.ergonomics },
            ].map((item) => (
              <div key={item.code} className="flex flex-col items-center">
                <Gauge value={item.value ?? 0} label={item.label} color={gaugeColor(item.value)} size="sm" />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex gap-3">
              ...
              <div className="space-y-0.5">
                {audits.filter((a) => a.status === "DRAFT" || a.status === "OPEN").slice(0, 5).map((a: any) => (
                  ...
                  >
                    ...
                    <span className="text-muted-foreground shrink-0 hidden sm:inline text-[10px]">{auditTypeName(a.auditType) || a.auditType || "—"}</span>
        ...
        <div className="flex-1 min-w-0 space-y-3" style={{ flexBasis: "40%" }}>
          ...
          <div className="bg-background/60 dark:bg-slate-900/60 backdrop-blur-md border border-white/30 dark:border-slate-700/30 p-3">
            ...
            {problems.filter((p) => p.status !== "CLOSED" && p.status !== "CANCELLED").length > 0 && (
              <div className="mt-2 pt-2 border-t border-white/10 dark:border-slate-700/10">
                <p className="text-[10px] text-muted-foreground">
                  {problems.filter((p) => p.status !== "CLOSED" && p.status !== "CANCELLED").length} open issue{problems.filter((p) => p.status !== "CLOSED" && p.status !== "CANCELLED").length > 1 ? "s" : ""} total
                </p>
              </div>
            )}
          </div>
